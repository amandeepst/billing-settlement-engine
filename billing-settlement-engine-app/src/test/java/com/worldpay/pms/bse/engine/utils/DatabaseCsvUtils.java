...
public class DatabaseCsvUtils {
  ...
  private static String createInsertStatement(String tableName, Seq<Column> columns, Record record) {
    Seq<Tuple2<String, String>> values = columns
          ...
          switch (column.getType().toLowerCase()) {
            ...
            case "timestamp":
              try {
                ...
                return Tuple.of(
                    name,
                    format("TIMESTAMP '%s'", Timestamp.valueOf(Date.valueOf(value).toLocalDate().atStartOfDay()))
                );
              }
            default:
    ...
    return format(
        ...
        tableName,
        values.map(Tuple2::_1).mkString(","),
        values.map(Tuple2::_2).mkString(",")
    );
  }

  public static <T> void checkThatDataFromCsvFileIsTheSameAsThatInList(String fileName, List<T> dataset, Class<T> type) {
    List<Record> records = getCsvRecords(fileName);
    records.remove(0); // remove header record

    List<Record> notMatch = records.stream().filter(record -> {
      boolean found = false;
    ...
    assertTrue(notMatch.isEmpty(), "The following records were not found in dataset" + notMatch.toString());
  }
  ...
  private static <T> boolean objectHasAllTheValues(Record record, T object, Class<T> type) {
    ...
    return fieldsMap.entrySet().stream()
        .allMatch(entry -> {
          try {
            Object actual = type.getMethod("get" + entry.getKey().substring(0, 1).toUpperCase() +
                entry.getKey().substring(1)).invoke(object);
            if(actual != null) {
  ...
  private static boolean compareValues(Object actual, String expected) {
    ...
    //the StringConverter in Sql2o is trimming the strings so we will get the data without the spaces.
    ...
    if (actual instanceof BigDecimal) {
      ...
      expectedAsObject = Date.valueOf(expected).toLocalDate();
  ...
  public static void readFromCsvFileAndDeleteFromTable(SqlDb db, String fileName, String tableName, String keyColumn) {
    List<Record> records = getCsvRecords(fileName);
    records.remove(0); // remove header record

    db.exec("delete " + tableName, conn -> {
      records.forEach(record -> {
        String deleteStatement = createDeleteStatement(tableName, keyColumn, record.getString(keyColumn));
  ...
  public static class Database {
    ...
    private static final Set<String> DB_WHITELIST = HashSet.of(
        "cdbd_eds_teoci241_app.worldpaytd.local",
        "cdbf_eds_piticasa318_app.worldpaytd.local",
        "cdbn_eds_toncum978_app.worldpaytd.local",
        "cdbs_eds_stefanescun601_app.worldpaytd.local",
        "cdbu_eds_arhipc998_app.worldpaytd.local",
        "cdbr_eds_dosuleanur363_app.worldpaytd.local",
        "cdbm_eds_miud782_app.worldpaytd.local",
        "cdbo_eds_mpg2_sandbox1_app.worldpaytd.local",
        "cdbq_eds_mpg2_sandbox3_app.worldpaytd.local",
        "cdbi_eds_jenkins_review_app.worldpaytd.local",
        "cdbj_eds_jenkins_ci_app.worldpaytd.local"
    );
    ...
    private void setup(String... tables) {
      clean(tables);
      Array.of(tables).forEach(table ->
          readFromCsvFileAndWriteToExistingTable(
              connection,
              Paths.get(basePath, table + ".csv").toString(),
              table)
      );

      // refresh materialized views
      connection.execQuery("refresh vwm_billing_account", "{call CBE_PCE_OWNER.PKG_PRICING_ENGINE.prc_refresh_billing_account()}",
    ...
    private void clean(String... tables) {
      DB_WHITELIST
          ...
          .getOrElseThrow(() -> new RuntimeException(format("Tried to run TRUNCATE on a non whitelisted DB instance %s.", conf.getUrl())));
